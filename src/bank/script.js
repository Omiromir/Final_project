...
window.onload = function () {
  // Fetch accounts from localStorage, defaulting to the predefined accounts if none exist
  const storedAccounts = JSON.parse(localStorage.getItem("accounts")) || accounts;
  accounts.length = 0; // Clear the current accounts array
  storedAccounts.forEach(acc => accounts.push(acc)); // Populate accounts with storedAccounts
  ...
  if (storedUser) {
    ...
    window.location.href = "../login/index.html"; // Redirect to login if no user is stored
  }
